...
| LI-004 | Inspect transactions with deterministic rules before LLM explanation. Each rule returns a structured `RuleFinding` with `rule_id`, `severity`, `reason`, and `relevant_fields`. | `src/banking_ai/rules.py`, `src/banking_ai/models.py`, `src/banking_ai/graph.py`, `src/banking_ai/consumer.py` | `tests/test_rules.py`, `pytest` | [#4](https://github.com/thomassuedbroecker/simple_kafka_example/issues/4) |
...
| LI-007 | Use local-only Ollama through direct HTTP API calls and stream output to the terminal. | `src/banking_ai/ollama_client.py`, `src/banking_ai/consumer.py` | Manual: `ollama serve`, `ollama pull qwen3-coder:30b`, consumer run | [#7](https://github.com/thomassuedbroecker/simple_kafka_example/issues/7) |
...
| LI-012 | Use a local Results UI to inspect the selected Kafka topic event, AI agent steps, deterministic findings, and streamed AI explanations in a browser. The UI can consume the next live message via a Kafka consumer group (showing partition and offset and committing after inspection) or replay a predefined demo event. | `src/banking_ai/results_ui.py`, `scripts/start_results_ui.sh`, `README.md` | `tests/test_results_ui.py`, manual: open `http://127.0.0.1:8081` | [#22](https://github.com/thomassuedbroecker/simple_kafka_example/issues/22), [#23](https://github.com/thomassuedbroecker/simple_kafka_example/issues/23), [#26](https://github.com/thomassuedbroecker/simple_kafka_example/issues/26) |
| LI-013 | Document local execution with Rancher Desktop as the only container runtime, with Kubernetes out of scope, and record key design decisions as ADRs. | `docs/local-execution.md`, `docs/troubleshooting.md`, `docs/adr/0001-local-first-rancher-desktop.md`, `docs/adr/0002-deterministic-rules-before-llm.md`, `docs/adr/0003-kafka-learning-scope.md`, `docs/adr/0004-no-kubernetes.md`, `README.md`, `scripts/verify.sh`, `scripts/demo.sh` | `./scripts/verify.sh`, manual local run | [#25](https://github.com/thomassuedbroecker/simple_kafka_example/issues/25) |
| LI-014 | Return structured rule findings with a `severity` level (`low`/`medium`/`high`) surfaced in the prompt and Results UI. | `src/banking_ai/models.py`, `src/banking_ai/rules.py`, `src/banking_ai/graph.py`, `src/banking_ai/results_ui.py` | `tests/test_rules.py::test_findings_carry_a_severity`, `pytest` | [#25](https://github.com/thomassuedbroecker/simple_kafka_example/issues/25) |
...
- Topic-event and AI-agent browser visibility maps to `results_ui.py` and the #23 Results UI enhancement.
...
- `results_ui.py` exists to satisfy LI-012 by showing the selected topic event, the AI agent steps, and the existing inspection workflow in a browser.
...
- [#23](https://github.com/thomassuedbroecker/simple_kafka_example/issues/23): `src/banking_ai/results_ui.py`, `tests/test_results_ui.py`, `README.md`, `docs/traceability.md`
